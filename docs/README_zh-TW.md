...
穩定版本HImGuiImageManager 是一個為 Dear ImGui 設計的 C++ 圖像管理庫，旨在簡化圖像的載入、顯示和管理流程。它支援從檔案、記憶體或 URL 載入圖像，並提供 GIF 動畫支援、圖像按鈕和自訂繪製功能，非常適合用於構建動態且視覺豐富的 ImGui 介面。
...
複製儲存庫：  git clone https://github.com/Half-People/HImGuiImageManager.git


加入專案：將 HImGuiImageManager.h（和 .cpp 若有）複製到您的專案中。  
連結相依性：確保已連結 ImGui 庫和您的圖形 API（如 OpenGL、DirectX）。  
編譯：將庫檔案編譯進您的專案。
...
相依性：需要 cpp-httplib 庫。  
下載：從 cpp-httplib GitHub 取得最新版本。  
...
相依性：需要 OpenSSL 3.0 或更高版本。  
下載：從 OpenSSL GitHub 取得源碼。  
建置：參考 OpenSSL 安裝指南。  
教學資源：若安裝過程有困難，建議觀看 此 YouTube 教學影片，提供詳細的 OpenSSL 配置說明。
...
📄 授權
HImGuiImageManager 採用 MIT 授權。您可以自由使用、修改和分發。

📧 聯繫與支援

GitHub：提交問題至 HImGuiImageManager Issues。  
電子郵件：請參考儲存庫中的聯繫資訊。
